...
typedef struct {
    ...
    XPlus_test_uio_map maps[ MAX_UIO_MAPS ];
} XPlus_test_uio_info;

/***************** Variable Definitions **************************************/
static XPlus_test_uio_info uio_info;
...
static int uio_info_read_name(XPlus_test_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPlus_test_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPlus_test_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPlus_test_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPlus_test_Initialize(XPlus_test *InstancePtr, const char* InstanceName) {
	XPlus_test_uio_info *InfoPtr = &uio_info;
